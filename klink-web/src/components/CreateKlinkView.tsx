...
const CreateKlinkView: Component<{ onSubmit: (value: { name: string, description?: string }) => void }> = (props) => {
    ...
    <form onSubmit={onSubmit}>
      <div class="flex flex-col w-full space-y-4 border-base-300 border-b-8 px-4 pt-2 pb-4">
        ...
        <input
          ...
          required
          value={form()}
          onInput={(e) => setForm(e.currentTarget.value)} />
        {/* Button Row */}
        <div class="flex flex-row items-center justify-between pl-2">
          {/* Create Modal Button */}
          <div class="tooltip tooltip-right hover:cursor-pointer" data-tip="Add a description">
            <CreateKlinkModal onSubmit={props.onSubmit}>
              {open =>
                <button class="btn btn-sm btn-circle btn-ghost" type="button" onClick={open}>
                  <Text size={24} />
                </button>
              }
            </CreateKlinkModal>
